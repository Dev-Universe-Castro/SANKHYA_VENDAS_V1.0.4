...
export class AccessControlService {
  ...
  getLeadsWhereClause(userAccess: UserAccess): { clause: string; binds: Record<string, any> } {
    ...
    // Vendedor comum: ver apenas seus leads (CODUSUARIO)
    ...
    // Gerente: ver leads seus e da equipe (buscar CODUSUARIOs da equipe)
    // Precisa buscar todos os usuários que possuem CODVEND da equipe
    return {
      clause: `AND (l.CODUSUARIO = :userId OR EXISTS (
        SELECT 1 FROM AD_USUARIOSVENDAS uv 
        WHERE uv.CODUSUARIO = l.CODUSUARIO 
        AND uv.CODVEND IN (${userAccess.vendedoresEquipe.join(',')})
        AND uv.ID_EMPRESA = :idEmpresa
      ))`,
      binds: { userId: userAccess.userId, idEmpresa: userAccess.idEmpresa }
    };
  }
  ...
  getPedidosWhereClause(userAccess: UserAccess): { clause: string; binds: Record<string, any> } {
    ...
    // Vendedor comum: ver apenas seus pedidos via CODVEND
    if (!userAccess.vendedoresEquipe || userAccess.vendedoresEquipe.length === 0) {
      if (!userAccess.codVendedor) {
        return { clause: '', binds: {} };
      }
      return {
        clause: 'AND cab.CODVEND = :codVend',
        binds: { codVend: userAccess.codVendedor }
      };
    }

    // Gerente: ver pedidos seus e da equipe
    const vendedores = [userAccess.codVendedor, ...userAccess.vendedoresEquipe].filter(Boolean);
    return {
      clause: `AND cab.CODVEND IN (${vendedores.join(',')})`,
      binds: {}
    };
  }
  ...
  getFinanceiroWhereClause(userAccess: UserAccess): { clause: string; binds: Record<string, any> } {
    ...
    // Vendedor comum: ver títulos de seus clientes via CODVEND
    if (!userAccess.vendedoresEquipe || userAccess.vendedoresEquipe.length === 0) {
      if (!userAccess.codVendedor) {
        return { clause: '', binds: {} };
      }
      return {
        clause: 'AND EXISTS (SELECT 1 FROM AS_PARCEIROS p WHERE p.CODPARC = fin.CODPARC AND p.ID_SISTEMA = fin.ID_SISTEMA AND p.CODVEND = :codVend)',
        binds: { codVend: userAccess.codVendedor }
      };
    }

    // Gerente: ver títulos de clientes seus e da equipe
    const vendedores = [userAccess.codVendedor, ...userAccess.vendedoresEquipe].filter(Boolean);
    return {
      clause: `AND EXISTS (SELECT 1 FROM AS_PARCEIROS p WHERE p.CODPARC = fin.CODPARC AND p.ID_SISTEMA = fin.ID_SISTEMA AND p.CODVEND IN (${vendedores.join(',')}))`,
      binds: {}
    };
  }
  ...
  getAtividadesWhereClause(userAccess: UserAccess): { clause: string; binds: Record<string, any> } {
    ...
    // Vendedor comum: ver apenas suas atividades (CODUSUARIO)
    if (!userAccess.vendedoresEquipe || userAccess.vendedoresEquipe.length === 0) {
      return {
    ...
    // Gerente: ver atividades suas e da equipe (buscar CODUSUARIOs da equipe)
    return {
      clause: `AND (a.CODUSUARIO = :userId OR EXISTS (
        SELECT 1 FROM AD_USUARIOSVENDAS uv 
        WHERE uv.CODUSUARIO = a.CODUSUARIO 
        AND uv.CODVEND IN (${userAccess.vendedoresEquipe.join(',')})
        AND uv.ID_EMPRESA = :idEmpresa
      ))`,
      binds: { userId: userAccess.userId, idEmpresa: userAccess.idEmpresa }

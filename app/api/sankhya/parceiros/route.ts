...
export async function GET(request: Request) {
  try {
    ...
    try {
      ...
      const search = searchParams.get('search') || ''; // Assumindo que 'search' é um parâmetro geral de busca
      const limit = parseInt(searchParams.get('limit') || '50'); // 'limit' como tamanho da página
